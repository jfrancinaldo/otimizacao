...
use mysql_common::{
    named_params::MixedParamsError, packets, params::MissingNamedParameterError, proto::codec::error::PacketCodecError,
    row::convert::FromRowError, value::convert::FromValueError,
};
use url::ParseError;
...
impl Error {
    ...
    pub fn is_connectivity_error(&self) -> bool {
        match self {
            ...
            Error::MySqlError(_) | Error::UrlError(_) | Error::FromValueError(_) | Error::FromRowError(_) => false,
        }
    }

    #[doc(hidden)]
    pub fn server_disconnected() -> Self {
        Error::IoError(io::Error::new(io::ErrorKind::BrokenPipe, "server disconnected"))
    }
}
...
impl fmt::Display for DriverError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            DriverError::MixedParams => write!(f, "Can not mix named and positional parameters in one statement"),
            DriverError::UnknownAuthPlugin(ref name) => {
                write!(f, "Unknown authentication protocol: `{}`", name)
            }
            DriverError::OldMysqlPasswordDisabled => {
                write!(f, "`old_mysql_password` plugin is insecure and disabled by default",)
            }
        }
    }
}
