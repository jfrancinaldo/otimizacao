...
impl Stream {
    pub fn make_secure(self, host: url::Host, ssl_opts: SslOpts) -> Result<Stream> {
        ...
        let server_name = domain.as_str().try_into().map_err(|_| webpki::InvalidDnsNameError)?;
        ...
        match self {
            Stream::TcpStream(tcp_stream) => match tcp_stream {
                TcpStream::Insecure(insecure_stream) => {
                    let inner = insecure_stream.into_inner().map_err(io::Error::from).unwrap();
                    ...
                    Ok(Stream::TcpStream(TcpStream::Secure(BufStream::new(secure_stream))))
                }
...
impl DangerousVerifier {
    fn new(accept_invalid_certs: bool, skip_domain_validation: bool, verifier: WebPkiVerifier) -> Self {
        Self {
    ...
    fn verify_server_cert(
        ...
        if self.accept_invalid_certs {
                ...
                .verify_server_cert(end_entity, intermediates, server_name, scts, ocsp_response, now)
            {
                ...
                Err(ref e) if e.to_string().contains("CertNotValidForName") && self.skip_domain_validation => {
                    Ok(rustls::client::ServerCertVerified::assertion())
                }
                Err(e) => Err(e),
            }
