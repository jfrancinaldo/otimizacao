...
impl<T: ToSocketAddrs> MyTcpBuilder<T> {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos",))]
    pub fn keepalive_probe_interval_secs(&mut self, keepalive_probe_interval_secs: Option<u32>) -> &mut Self {
    ...
    pub fn connect(self) -> io::Result<TcpStream> {
        ...
        let socket = if let Some(bind_address) = bind_address {
            ...
            // no bind address
            addrs.into_iter().fold(Err(err), |prev, sock_addr| match prev {
                Ok(socket) => Ok(socket),
                Err(_) => {
                    let domain = Domain::for_address(sock_addr);
                    let socket = Socket::new(domain, Type::STREAM, None)?;
        ...
        if let Some(duration) = keepalive_time_ms {
            let conf = socket2::TcpKeepalive::new().with_time(Duration::from_millis(duration as u64));
