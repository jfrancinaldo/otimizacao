...
use mysql_common::{
    ...
    packets::{
        ...
        ComStmtExecuteRequestBuilder, ComStmtSendLongData, CommonOkPacket, ErrPacket, HandshakePacket,
        HandshakeResponse, OkPacket, OkPacketDeserializer, OkPacketKind, OldAuthSwitchRequest, ResultSetTerminator,
        SessionStateInfo, SslRequest,
    },
...
use crate::{
    ...
    prelude::*,
    DriverError::{
        MismatchedStmtParams, NamedParamsForPositionalQuery, OldMysqlPasswordDisabled, Protocol41NotSet,
...
impl Conn {
    ...
    pub fn warnings(&self) -> u16 {
        self.0.ok_packet.as_ref().map(OkPacket::warnings).unwrap_or_default()
    }
    ...
    pub fn reset(&mut self) -> Result<()> {
        match (self.0.server_version, self.0.mariadb_server_version) {
            (Some(ref version), _) if *version > (5, 7, 3) => self.soft_reset().or_else(|_| self.hard_reset()),
            (_, Some(ref version)) if *version >= (10, 2, 7) => self.soft_reset().or_else(|_| self.hard_reset()),
            _ => self.hard_reset(),
        }
    }
    ...
    #[inline(always)]
    fn handle_ok<'a, T: OkPacketKind>(&mut self, buffer: &'a Buffer) -> crate::Result<OkPacket<'a>> {
    ...
    fn more_results_exists(&self) -> bool {
        self.0.status_flags.contains(StatusFlags::SERVER_MORE_RESULTS_EXISTS)
    }

    #[inline(always)]
    fn perform_auth_switch(&mut self, auth_switch_request: AuthSwitchRequest<'_>) -> Result<()> {
        if matches!(auth_switch_request.auth_plugin(), AuthPlugin::MysqlOldPassword) {
    ...
    fn do_handshake(&mut self) -> Result<()> {
        ...
        if self.0.capability_flags.contains(CapabilityFlags::CLIENT_COMPRESS) {
    ...
    fn switch_to_compressed(&mut self) {
        self.stream_mut().codec_mut().compress(Compression::default());
    }
    ...
    #[inline(always)]
    fn write_handshake_response(&mut self, auth_plugin: &AuthPlugin<'_>, scramble_buf: Option<&[u8]>) -> Result<()> {
    ...
    #[inline(always)]
    fn continue_auth(&mut self, auth_plugin: &AuthPlugin<'_>, nonce: &[u8], auth_switched: bool) -> Result<()> {
        match auth_plugin {
    ...
    #[inline(always)]
    fn continue_mysql_native_password_auth(&mut self, nonce: &[u8], auth_switched: bool) -> Result<()> {
        let payload = self.read_packet()?;
    ...
    #[inline(always)]
    fn continue_caching_sha2_password_auth(&mut self, nonce: &[u8], auth_switched: bool) -> Result<()> {
        let payload = self.read_packet()?;
            ...
            0x01 => match payload[1] {
                ...
                0x04 => {
                    if !self.is_insecure() || self.is_socket() {
                        let mut pass = self.0.opts.get_pass().map(Vec::from).unwrap_or_else(Vec::new);
                        pass.push(0);
    ...
    fn send_long_data(&mut self, stmt_id: u32, params: &[Value]) -> Result<()> {
        for (i, value) in params.iter().enumerate() {
            if let Bytes(bytes) = value {
                ...
                let chunks = chunks.chain(if bytes.is_empty() { Some(&[][..]) } else { None });
                for chunk in chunks {
    ...
    #[inline(always)]
    fn _execute(&mut self, stmt: &Statement, params: Params) -> Result<Or<Vec<Column>, OkPacket<'static>>> {
    ...
    fn _start_transaction(&mut self, tx_opts: TxOpts) -> Result<()> {
        ...
        if tx_opts.with_consistent_snapshot() {
            self.query_drop("START TRANSACTION WITH CONSISTENT SNAPSHOT").unwrap();
        } else {
    ...
    fn send_local_infile(&mut self, file_name: &[u8]) -> Result<OkPacket<'static>> {
        {
            let buffer_size = cmp::min(MAX_PAYLOAD_LEN - 4, self.stream_ref().codec().max_allowed_packet - 4);
    ...
    fn _prepare(&mut self, query: &str) -> Result<Arc<InnerStmt>> {
        ...
        if let Some(old_stmt) = self.0.stmt_cache.put(Arc::new(query.into()), inner_st.clone()) {
    ...
    fn connect(&mut self) -> Result<()> {
        ...
        self.do_handshake()
            .and_then(|_| {
                Ok(from_value_opt::<usize>(self.get_system_var("max_allowed_packet")?.unwrap_or(NULL)).unwrap_or(0))
            })
...
mod test {
    mod my_conn {
        ...
        fn should_connect() {
            ...
            let mode: String = conn.query_first("SELECT @@GLOBAL.sql_mode").unwrap().unwrap();
        ...
        fn should_select_db() {
            ...
            conn.query_drop(format!("DROP DATABASE {}", DB_NAME)).unwrap();
        }
        ...
        fn should_parse_large_text_result() {
            let mut conn = Conn::new(get_opts()).unwrap();
            let value: Value = conn.query_first("SELECT REPEAT('A', 20000000)").unwrap().unwrap();
        ...
        fn should_start_commit_and_rollback_transactions() {
            let mut conn = Conn::new(get_opts()).unwrap();
            conn.query_drop("CREATE TEMPORARY TABLE mysql.tbl(id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, a INT)")
                .unwrap();
            let _ = conn
            ...
            tx.exec_drop("INSERT INTO mysql.tbl(a) VALUES(?)", (3,)).unwrap();
            tx.exec_drop("INSERT INTO mysql.tbl(a) VALUES(?)", (4,)).unwrap();
            tx.commit().unwrap();
            assert_eq!(
                conn.query_iter("SELECT COUNT(a) from mysql.tbl")
                    .unwrap()
            ...
            let mut tx = conn.start_transaction(TxOpts::default()).unwrap();
            tx.exec_drop("INSERT INTO mysql.tbl(a) VALUES(?)", (5,)).unwrap();
            tx.exec_drop("INSERT INTO mysql.tbl(a) VALUES(?)", (6,)).unwrap();
            drop(tx);
            assert_eq!(
        ...
        fn should_reset_connection() {
            ...
            conn.reset().unwrap();
            assert_eq!(conn.affected_rows(), 0);
            conn.query_drop("SELECT * FROM `mysql`.`test`;").unwrap_err();
        }
        ...
        fn prep_exec() {
            ...
            assert_eq!(
                conn.exec::<String, _, _>(&stmt1, params! { "foo" => "foo" }).unwrap(),
                vec![String::from("foo")],
            );
            assert_eq!(
                conn.exec::<String, _, _>(&stmt2, params! { "bar" => "bar" }).unwrap(),
                vec![String::from("bar")],
            );
        }
        ...
        fn issue_273() {
            ...
            "SELECT f1(?, ?)".with((100u8, 100u8)).run(&mut conn).unwrap();
        }
        ...
        fn should_return_error_on_missing_named_parameter() {
            ...
            let result = conn.exec_first::<crate::Row, _, _>(&stmt, params! {"a" => 1, "b" => 2, "c" => 3,});
            match result {
        ...
        fn should_set_additional_capabilities() {
            ...
            let opts = OptsBuilder::from_opts(get_opts()).additional_capabilities(CapabilityFlags::CLIENT_FOUND_ROWS);
            ...
            conn.query_drop("INSERT INTO mysql.tbl (a, b) VALUES (1, 'foo')")
                .unwrap();
            let result = conn.query_iter("UPDATE mysql.tbl SET b = 'foo' WHERE a = 1").unwrap();
        ...
        fn should_handle_json_columns() {
            ...
            let (a, b): (String, Json) = conn.query_first("SELECT a, b FROM mysql.tbl").unwrap().unwrap();
            assert_eq!(
                (a, b),
                (
                    "hello".into(),
        ...
        fn should_set_connect_attrs() {
            ...
            if support_connect_attrs {
                ...
                let attrs_size: i32 = get_system_variable(&mut conn, "performance_schema_session_connect_attrs_size");
                ...
                fn assert_connect_attrs(conn: &mut Conn, expected_values: &[(&str, &str)]) {
                    ...
                    for (name, value) in expected_values {
                        assert_eq!(actual_values.get(&name.to_string()), Some(&value.to_string()));
                    }
                }

                let pid = process::id().to_string();
                let progname = std::env::args_os().next().unwrap().to_string_lossy().into_owned();
        ...
        fn should_read_binlog() -> crate::Result<()> {
            use std::{collections::HashMap, sync::mpsc::sync_channel, thread::spawn, time::Duration};
            ...
            fn get_conn() -> crate::Result<(Conn, Vec<u8>, u64)> {
                ...
                if let Ok(Some(gtid_mode)) = "SELECT @@GLOBAL.GTID_MODE".first::<String, _>(&mut conn) {
    ...
    mod bench {
        ...
        fn simple_prepared_query_row_with_5_named_params(bencher: &mut test::Bencher) {
            let mut conn = Conn::new(get_opts()).unwrap();
            let stmt = conn.prep("SELECT :one, :two, :three, :four, :five").unwrap();
            bencher.iter(|| {
        ...
        fn many_small_rows(bencher: &mut test::Bencher) {
            let mut conn = Conn::new(get_opts()).unwrap();
            conn.query_drop("CREATE TEMPORARY TABLE mysql.x (id INT)").unwrap();
            for _ in 0..512 {
