...
use crate::{
    ...
    prelude::*,
    Conn, DriverError, Error, LocalInfileHandler, Opts, Params, QueryResult, Result, Statement, Transaction, TxOpts,
};
...
impl Drop for PooledConn {
    fn drop(&mut self) {
        if self.pool.arced_pool.count.load(Ordering::Relaxed) > self.pool.arced_pool.max || self.conn.is_none() {
...
impl PooledConn {
    ...
    pub fn get_binlog_stream(mut self, request: crate::BinlogRequest<'_>) -> Result<crate::BinlogStream> {
...
mod test {
    mod pool {
        ...
        use crate::{from_value, prelude::*, test_misc::get_opts, DriverError, Error, OptsBuilder, Pool, TxOpts};
        ...
        fn multiple_pools_should_work() {
            ...
            pool.get_conn()
                ...
                .unwrap();
            pool.get_conn().unwrap().exec_drop("CREATE DATABASE A", ()).unwrap();
            pool.get_conn()
                .unwrap()
            ...
            let count: u8 = pool2
                ...
                .unwrap();
            assert_eq!(1, count);
            pool.get_conn().unwrap().exec_drop("DROP DATABASE A", ()).unwrap();
        }
        ...
        fn should_fix_connectivity_errors_on_prepare() {
            ...
            pool.get_conn().unwrap().prep("SHOW FULL PROCESSLIST").unwrap();
        }
        ...
        fn should_fix_connectivity_errors_on_prep_exec() {
            ...
            conn.query_drop(&*format!("KILL {}", id)).unwrap();
            thread::sleep(Duration::from_millis(250));
            pool.get_conn().unwrap().exec_drop("SHOW FULL PROCESSLIST", ()).unwrap();
        }
        #[test]
        ...
        fn should_start_transaction_on_PooledConn() {
            let pool = Pool::new(get_opts()).unwrap();
            let mut conn = pool.get_conn().unwrap();
            conn.query_drop("CREATE TEMPORARY TABLE mysql.tbl(a INT)").unwrap();
