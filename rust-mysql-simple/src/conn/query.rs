...
pub trait TextQuery: Sized {
    ...
    where
        ...
        T: FromRow,
    {
        self.run(conn)?.next().map(|row| row.map(from_row_opt)).transpose()
    }
    ...
    {
        self.fold_opt(conn, Vec::new(), |mut acc, row: StdResult<T, FromRowError>| {
            acc.push(map(row));
            acc
        })
    }
...
impl<T: AsRef<str>> WithParams for T {
    fn with<P>(self, params: P) -> QueryWithParams<Self, P> {
        QueryWithParams { query: self, params }
    }
}
