...
use std::{borrow::Cow, collections::HashMap, hash::Hash, net::SocketAddr, path::Path, time::Duration};
...
impl SslOpts {
    ...
    pub fn with_root_cert_path<T: Into<Cow<'static, Path>>>(mut self, root_cert_path: Option<T>) -> Self {
...
impl OptsBuilder {
    ...
    pub fn from_hash_map(mut self, client: &HashMap<String, String>) -> Result<Self, UrlError> {
        for (key, value) in client.iter() {
            match key.as_str() {
                ...
                "host" => {
                    let host = url::Host::parse(value).unwrap_or_else(|_| url::Host::Domain(value.to_owned()));
                    self.opts.0.ip_or_hostname = host;
                }
                "port" => match value.parse::<u16>() {
                    Ok(parsed) => self.opts.0.tcp_port = parsed,
                    Err(_) => return Err(UrlError::InvalidValue(key.to_string(), value.to_string())),
                },
    ...
    pub fn ip_or_hostname<T: Into<String>>(mut self, ip_or_hostname: Option<T>) -> Self {
        ...
        self.opts.0.ip_or_hostname = url::Host::parse(&new).unwrap_or_else(|_| url::Host::Domain(new.to_owned()));
        self
    }
    ...
    /// Can be defined using `tcp_keepalive_probe_interval_secs` connection url parameter.
    #[cfg(any(target_os = "linux", target_os = "macos",))]
    pub fn tcp_keepalive_probe_interval_secs(mut self, tcp_keepalive_probe_interval_secs: Option<u32>) -> Self {
...
fn from_url(url: &str) -> Result<Opts, UrlError> {
    ...
    OptsBuilder::from_opts(opts).from_hash_map(&hash_map).map(Into::into)
}
...
mod test {
    ...
    fn should_read_hashmap_into_opts() {
        ...
        #[cfg(any(target_os = "linux", target_os = "macos",))]
        cnf_map.insert("tcp_keepalive_probe_interval_secs".to_string(), "8".to_string());
        #[cfg(any(target_os = "linux", target_os = "macos",))]
        ...
        #[cfg(any(target_os = "linux", target_os = "macos",))]
        assert_eq!(parsed_opts.opts.get_tcp_keepalive_probe_interval_secs(), Some(8));
        #[cfg(any(target_os = "linux", target_os = "macos",))]
        assert_eq!(parsed_opts.opts.get_tcp_keepalive_probe_count(), Some(5));
        assert_eq!(parsed_opts.opts.get_compress(), Some(crate::Compression::best()));
        assert_eq!(
    ...
    fn should_have_url_err() {
        use crate::{OptsBuilder, UrlError};
        macro_rules!  map(
            { $($key:expr => $value:expr), + }=> {
                {
                    let mut h = std::collections::HashMap::new();
        ...
        assert_eq!(
            parsed,
            Err(UrlError::InvalidValue("port".to_string(), "NOTAPORT".to_string()))
        );
    }
}
